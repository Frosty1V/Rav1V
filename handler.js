        ...
        for (const user of participants) {
          ...
          try {
            ...
            const isBotAdminNn = botTt2?.admin === 'admin' || false;
               text = (action === 'add' ? (chat.sWelcome || tradutor.texto1 || conn.welcome || 'Welcome, @user!').replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*بدون وصف*') :
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '𝑅𝒶𝓋', contacts: [{vcard}]}}, {quoted: callmsg});
...
const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {thumbnail: imagen1}}}}, aa);
